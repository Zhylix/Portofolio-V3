@php
    $paletteProjects = \App\Models\Project::ordered()->take(6)->get();
    $paletteExperiences = \App\Models\Experience::published()->ordered()->take(4)->get();
    $paletteArticles = \App\Models\Article::published()->ordered()->take(4)->get();
@endphp
...
<div 
    x-data="{
        ...
        easterEgg: false,
        terminalOutput: [
            'Helmy Yunan OS v4.1.0-alpha [System Kernel: Laravel 13.31.0 on PHP 8.4.22]',
            'Architecture: Distributed Multi-Tenant & Enterprise Microservices',
            'Type: help, neofetch, clear, or exit to interact.'
        ],
        terminalInput: '',
        navItems: [
            { title: 'Home', subtitle: 'Overview & system landing', url: '{{ route('home') }}', badge: 'Nav' },
            { title: 'About Helmy', subtitle: 'Architecture philosophy & background', url: '{{ route('about') }}', badge: 'Bio' },
            { title: 'Projects & Systems', subtitle: 'Case studies & production work', url: '{{ route('projects.index') }}', badge: 'Work' },
            { title: 'Professional Journey', subtitle: 'Career timeline & leadership milestones', url: '{{ route('journey.index') }}', badge: 'Journey' },
            { title: 'Capabilities & Skills', subtitle: 'Empirical backend & infrastructure stack', url: '{{ route('skills.index') }}', badge: 'Stack' },
            { title: 'Certifications', subtitle: 'AWS, Laravel & verified licenses', url: '{{ route('certificates.index') }}', badge: 'Certs' },
            { title: 'Achievements & Honors', subtitle: 'National hackathons & distinctions', url: '{{ route('achievements.index') }}', badge: 'Honors' },
            { title: 'Articles & Writings', subtitle: 'Technical publications & database guides', url: '{{ route('articles.index') }}', badge: 'Blog' },
            { title: 'Contact & Collaboration', subtitle: 'Start an inquiry or technical discussion', url: '{{ route('contact.index') }}', badge: 'Contact' },
        ],
        handleTerminalCommand() {
            ...
            if (cmd === 'help') {
                this.terminalOutput.push('Available: neofetch, skills, projects, clear, exit');
            } else if (cmd === 'neofetch') {
                this.terminalOutput.push('  ██████╗  Host: Helmy Yunan Nasution');
                this.terminalOutput.push('  ██╔══██╗ OS: Arch / Ubuntu Noble / Laravel 13');
                this.terminalOutput.push('  ██████╔╝ Kernel: PHP 8.4.22 + MySQL + Redis');
                this.terminalOutput.push('  ██╔══██╗ Uptime: 99.999% Reliability');
                this.terminalOutput.push('  ██████╔╝ Status: AVAILABLE FOR STRATEGIC ROLES');
            } else if (cmd === 'skills') {
                this.terminalOutput.push('Core Stack: Laravel, PHP 8.4, MySQL, Docker, Redis, Tailwind CSS 4, Livewire 4, Spatie Media');
            } else if (cmd === 'clear') {
                this.terminalOutput = [];
            } else if (cmd === 'exit' || cmd === 'quit') {
                this.easterEgg = false;
            } else {
                this.terminalOutput.push('command not found: ' + cmd + '. Type \"help\" for manual.');
            }
            this.terminalInput = '';
        }
    }"
    x-on:open-command-palette.window="isOpen = true; setTimeout(() => $refs.searchInput.focus(), 80)"
    x-on:keydown.escape.window="isOpen = false; easterEgg = false"
    style="display: none;"
    x-show="isOpen"
    ...
    >
        ...
        <div class="relative flex items-center px-4 border-b border-[#2A2520]">
            ...
            <input 
                ...
                @input="if(['terminal', 'neofetch', 'matrix', 'sudo'].includes(search.trim().toLowerCase())) { easterEgg = true; search = ''; }"
        ...
        <div x-show="easterEgg" class="p-5 bg-black/95 font-mono text-xs text-emerald-400 space-y-3 max-h-96 overflow-y-auto border-b border-[#2A2520]">
            <div class="flex items-center justify-between pb-2 border-b border-emerald-950 text-[11px] text-[#E47A2E]">
                ...
                <button @click="easterEgg = false" class="text-rose-400 hover:underline">close</button>
            </div>
        ...
        <div class="max-h-[60vh] overflow-y-auto p-4 space-y-6">
            ...
            <div class="space-y-1">
                ...
                <template x-for="item in navItems.filter(i => !search || i.title.toLowerCase().includes(search.toLowerCase()) || i.subtitle.toLowerCase().includes(search.toLowerCase()))" :key="item.title">
                    <a 
                        :href="item.url" 
                        class="flex items-center justify-between p-2.5 rounded-xl hover:bg-[#151311] border border-transparent hover:border-[#2A2520] transition group"
                    >
                        <div class="flex items-center gap-3">
                            <span class="w-1.5 h-1.5 rounded-full bg-[#C45A19] group-hover:scale-125 transition-transform"></span>
                            <div>
                                <p class="text-sm font-semibold text-[#F5F1EA] group-hover:text-[#E47A2E] transition-colors" x-text="item.title"></p>
                    ...
                    @foreach($paletteProjects as $proj)
                        ...
                        >
                            <div class="truncate max-w-md">
                                <p class="text-sm font-semibold text-[#F5F1EA] group-hover:text-[#E47A2E] transition-colors truncate">
                                    {{ $proj->title }}
                                </p>
            ...
            <!-- Articles Live Suggestions -->
            @if($paletteArticles->isNotEmpty())
                <div class="space-y-1 pt-2 border-t border-[#2A2520]">
                    <p class="text-[10px] font-mono uppercase tracking-widest text-[#70685F] px-3 mb-2">Technical Publications</p>
                    @foreach($paletteArticles as $art)
                        <a 
                        ...
                        >
                            <div class="truncate max-w-md">
                                ...
                                <p class="text-xs text-[#70685F] font-mono">{{ $art->category }}</p>
                            </div>
                            <span class="text-xs font-mono text-[#E47A2E] shrink-0">&rarr;</span>
                        </a>
                    @endforeach
        ...
        <div class="px-5 py-3 bg-[#080808] border-t border-[#2A2520] flex items-center justify-between text-[11px] font-mono text-[#70685F]">
            ...
            <span class="text-[#E47A2E]">Helmy Yunan System</span>
        </div>
    </div>
</div>
